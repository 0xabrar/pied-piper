...
import {INITIAL_STATE, GRANTED_STATE, REQUESTED_STATE, PENDING_STATE, REFUSED_STATE, ACCEPTED_STATE} from "../constants/tickets";
...
class ApplicantRatio extends React.Component {
  ...
  render() {
    if(this.props.tickets.filter((ticket) => ticket.state === this.props.state).length === 0) {
      return(
        <div>
          <Header textAlign='center' as='h3'>{getStateTitle(this.props.state)}</Header>
          <Header textAlign='center' as='h3' style={{marginTop: '100px'}}>No matching data.</Header>
        </div>
      )
    }
    else {
